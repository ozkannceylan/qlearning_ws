...
class MotionPlanner():
    ...
    @staticmethod
    def run_initialize(C,q_table_dir):
        ...
        return  actions,state_space,Q_table
    @staticmethod
    def print_init_pos(x,y,theta):
        print('\r\nInitial position:')
        print('x = %.2f [m]' % x)
        print('y = %.2f [m]' % y)
    ...
    #     ( self.x_in , self.y_in , self.theta_in ) = self.C.robotSetPos(self.setPosPub, self.x_init, self.y_init, self.theta_init)
    #     # check init pos
    ...
    #     self.status=self.robotFeedbackControl(self.velPub, self.x, self.y, self.theta, self.x_goal, self.y_goal, (self.theta_goal))
    ...
    #     self.x,self.y,self.theta=self.get_odom_params()
    ...
    #         self.status = self.robotFeedbackControl(self.velPub, self.x, self.y, self.theta, self.x_goal, self.y_goal, radians(self.theta_goal)) 
    #         self.text = self.text + ' ==> Feedback control algorithm '
    #         if self.goal_near:
    #             self.text = self.text + '(goal near)'
