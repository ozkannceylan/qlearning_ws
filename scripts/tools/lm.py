...
class MapLearner(Control):
    ...
    #     else:
    #         ( self.action, self.status_strat ) = self.epsiloGreedyExploration(self.Q_table, self.state_ind, self.actions, self.T)
    #     return self.action
    ...
    def process_action_step(self):
        ...
        new_state_ind=self.scan_env() 
        reward=self.get_reward_update_qtable() ### update with dqn
        self.get_action() ##update with dqn
        self.status_rda = self.robotDoAction(self.velPub, self.action)
        # self.update_prev_state()
        self.log_status()   
        self.update_prev_state()
        return new_state_ind, reward
    ...
    def step(self,action):
        ...
        return new_state_ind, self.reward
    ...
    def run_learner(self):
        ...
        for episode in tqdm(range(1, EPISODES + 1), unit= "episode"):
            ...
            if episode % STATS_EVERY == 0 or episode == 1:
                ...
                self.agent.tensorboard.update_stats(reward_avg=avg_reward, reward_min=min_reward, reward_max=max_reward)

                if avg_reward >= MIN_REWARD:
                    self.agent.model.save(f'models/{MODEL_NAME}__{max_reward:_>7.2f}max_{avg_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model')
